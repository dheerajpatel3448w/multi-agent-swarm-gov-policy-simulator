...
:
    sim_xxx/
    ├── twitter/
    │   └── actions.jsonl    # Twitter 
    ├── reddit/
    │   └── actions.jsonl    # Reddit 
    ├── simulation.log       # 
    └── run_state.json       # （API ）
"""

# ============================================================
#  Windows ： import  UTF-8 
#  OASIS 
# ============================================================
import sys
import os
            ...
            try:
                ...
                for interview in twitter_interviews:
                    ...
                    try:
                        ...
                        print(f"  : Twitter Agent {agent_id}: {e}")
            ...
            try:
                ...
                for interview in reddit_interviews:
                    ...
                    try:
                        ...
                        print(f"  : Reddit Agent {agent_id}: {e}")
...
def fetch_new_actions_from_db(
    ...
    Args:
        db_path: 
        last_rowid:  rowid （ rowid  created_at， created_at ）
        agent_names: agent_id -> agent_name 
        
    Returns:
        (actions_list, new_last_rowid)
        - actions_list: ， agent_id, agent_name, action_type, action_args（）
...
async def run_twitter_simulation(
    ...
    for round_num in range(total_rounds):
        ...
        if _shutdown_event and _shutdown_event.is_set():
            if main_logger:
                main_logger.info(f"， {round_num + 1} ")
            break
    ...
    log_info(f"! : {elapsed:.1f}, : {total_actions}")
...
async def main():
    ...
    log_manager.info(f"ID: {config.get('simulation_id', 'unknown')}")
    log_manager.info(f": {'' if wait_for_commands else ''}")
    log_manager.info("=" * 60)
    ...
    log_manager.info(f":")
    log_manager.info(f"  - : {total_hours}")
    log_manager.info(f"  - : {minutes_per_round}")
    log_manager.info(f"  - : {config_total_rounds}")
    if args.max_rounds:
        log_manager.info(f"  - : {args.max_rounds}")
        if args.max_rounds < config_total_rounds:
            log_manager.info(f"  - : {args.max_rounds} ()")
    log_manager.info(f"  - Agent: {len(config.get('agent_configs', []))}")
    
    log_manager.info(":")
    log_manager.info(f"  - : simulation.log")
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"! : {total_elapsed:.1f}")
