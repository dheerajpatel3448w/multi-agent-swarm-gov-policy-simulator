...
logger = get_logger('AGENTIXGOV.oasis_profile')
...
class OasisProfileGenerator:
    ...
    def generate_profile_from_entity(
        ...
        """
        ZepGenerateOASIS Agent Profile
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.debug(f"Zep {attempt + 1} Failed: {str(e)[:80]}, ...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"Zep {max_retries} Failed: {e}")
            return None
        ...
        try:
            ...
            logger.warning(f"ZepFailed ({entity_name}): {e}")
    ...
    def _generate_profile_with_llm(
        ...
        for attempt in range(max_attempts):
            try:
                ...
                try:
                    ...
                    logger.warning(f"JSONFailed (attempt {attempt+1}): {str(je)[:80]}")
        ...
        logger.warning(f"LLMGenerateFailed（{max_attempts}）: {last_error}, Generate")
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """"""
        base_prompt = "Generate。Generate、,。JSON，。"
...
- """
    ...
    def generate_profiles_from_entities(
        ...
        """
        GenerateAgent Profile（Generate）
        ...
        Args:
            ...
            graph_id: ID，Zep
            parallel_count: Generate，5
            realtime_output_path: （If，Generate）
        ...
        def save_profiles_realtime():
            ...
            with lock:
                ...
                try:
                    ...
                    logger.warning(f"Save profiles Failed: {e}")
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                logger.error(f"Generate {entity.name} Failed: {str(e)}")
                # profile
        ...
        logger.info(f"Generate {total} Agent（: {parallel_count}）...")
        print(f"\n{'='*60}")
        print(f"GenerateAgent -  {total} ，: {parallel_count}")
        print(f"{'='*60}\n")
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    if error:
                        ...
                        logger.info(f"[{current}/{total}] SuccessGenerate: {entity.name} ({entity_type})")
        ...
        print(f"\n{'='*60}")
        print(f"Generate！Generate {len([p for p in profiles if p])} Agent")
        print(f"{'='*60}\n")
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        logger.info(f"Save {len(profiles)} Twitter Profile {file_path} (OASIS CSV)")
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        logger.info(f"Save {len(profiles)} Reddit Profile {file_path} (JSON，user_id)")
