...
class SimulationRunner:
    ...
    2. Parsing run logs and recording each Agent action
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Maximum simulation rounds (optional, to limit long simulations)
        enable_graph_memory_update: bool = False,  # Whether to update activities to Zep graph
        graph_id: str = None  # ZepID（）
    ) -> SimulationRunState:
        """
        ...
        Args:
            ...
            max_rounds: Maximum simulation rounds (optional, to limit long simulations)
        ...
        # Determine which script to run (scripts in backend/scripts/ dir)
        if platform == "twitter":
        ...
        try:
            ...
            # Create main log file to avoid blocking process when stdout/stderr pipe buffer fills up
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # Set subprocess environment variables to ensure UTF-8 on Windows
            # Fixes encoding issues in third-party libraries (e.g. OASIS) that do not specify encoding
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+ support, makes all open() calls default to UTF-8
            env['PYTHONIOENCODING'] = 'utf-8'  # Ensure stdout/stderr use UTF-8
            
            # Set working directory to simulation dir (databases etc. created here)
            # Use start_new_session=True to create new process group, ensuring all subprocesses can be killed via os.killpg
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group to ensure all related processes are terminated when server shuts down
            )
            
            # Save file handles for later closing
            cls._stdout_files[simulation_id] = main_log_file
            cls._stderr_files[simulation_id] = None  # No longer needs separate stderr
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'zh'):
        ...
        try:
            ...
            # Final log read after process ends
            if os.path.exists(twitter_actions_log):
                ...
                try:
                    if os.path.exists(main_log_path):
                        with open(main_log_path, 'r', encoding='utf-8') as f:
                            error_info = f.read()[-2000:]  # Take last 2000 chars
                except Exception:
                    pass
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If two platforms ran, both must complete
                                ...
                                # Update round info from round_end event
                                    ...
                                    # Overall rounds is max of both platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Overall time is max of both platforms
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determines if a platform is enabled by checking if its actions.jsonl file exists
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        """
        Terminate process and its subprocesses (cross-platform)
        ...
        if IS_WINDOWS:
            ...
            logger.info(f"Terminate process (Windows): simulation={simulation_id}, pid={process.pid}")
            try:
                ...
                try:
                    ...
                    # Force terminate
                    logger.warning(f"，Force terminate: {simulation_id}")
                    subprocess.run(
            ...
            logger.info(f"Terminate process (Unix): simulation={simulation_id}, pgid={pgid}")
            ...
            try:
                ...
                # If timeout occurs and process has not ended, force send SIGKILL
                logger.warning(f" SIGTERM，Force terminate: {simulation_id}")
    ...
    def stop_simulation(cls, simulation_id: str) -> SimulationRunState:
        ...
        if process and process.poll() is None:
            try:
                ...
                logger.error(f"Terminate processFailed: {simulation_id}, error={e}")
                # Terminate process
                try:
                    process.terminate()
                    process.wait(timeout=5)
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer record platform field, fall back to default platform
    ...
    def get_all_actions(
        ...
        if not actions:
            ...
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # Old format file should have a platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
            ...
            if os.path.exists(dir_path):
                ...
                if os.path.exists(actions_file):
                    try:
                        ...
                        errors.append(f" {dir_name}/actions.jsonl Failed: {str(e)}")
            ...
            try:
                if process.poll() is None:  # Process still running
                    ...
                    try:
                        ...
                        logger.warning(f" state.json Failed: {simulation_id}, error={state_err}")
    ...
    def interview_agent(
        ...
        Raises:
            ValueError: Simulation does not exist
            TimeoutError: 
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
    ...
    def _get_interview_history_from_db(
        ...
        try:
            ...
            logger.error(f"InterviewFailed ({platform_name}): {e}")
