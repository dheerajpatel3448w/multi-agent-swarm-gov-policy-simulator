...
const phase = ref(0) // 0: , 1: , 2: Complete
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Complete（ runner_status CompleteStatus）
...
.actions-tooltip::before {
  ...
  border-bottom: 6px solid var(--border-glass);
}
...
.platform-status.twitter .platform-icon { color: var(--ocean-blue); }
.platform-status.reddit .platform-icon { color: var(--accent-orange); }
...
.breakdown-divider { color: var(--border-glass); }
.breakdown-item.twitter { color: var(--ocean-blue); }
.breakdown-item.reddit { color: var(--accent-orange); }
...
.timeline-marker {
  ...
  background: var(--bg-panel);
  border: 1px solid var(--ocean-blue);
  box-shadow: 0 0 5px var(--ocean-blue);
  border-radius: 50%;
  transform: translateX(-50%);
  z-index: 2;
  display: flex;
...
.timeline-item.twitter .marker-dot { background: var(--ocean-blue); }
.timeline-item.reddit .marker-dot { background: var(--accent-orange); }
.timeline-item.twitter .timeline-marker { border-color: var(--ocean-blue); box-shadow: 0 0 5px var(--ocean-blue); }
.timeline-item.reddit .timeline-marker { border-color: var(--accent-orange); box-shadow: 0 0 5px var(--accent-orange); }
...
.avatar-placeholder {
  ...
  font-weight: 700;
  text-transform: uppercase;
  text-shadow: 0 0 5px var(--ocean-blue);
}
...
.badge-post { background: rgba(0, 229, 255, 0.1); color: var(--ocean-blue); border-color: rgba(0, 229, 255, 0.3); }
.badge-comment { background: rgba(255, 153, 0, 0.1); color: var(--accent-orange); border-color: rgba(255, 153, 0, 0.3); }
.badge-action { background: rgba(255, 255, 255, 0.05); color: var(--text-primary); border: 1px solid var(--border-glass); }
.badge-meta { background: transparent; color: var(--text-secondary); border: 1px dashed var(--border-glass); }
.badge-idle { opacity: 0.5; color: var(--text-secondary); border: 1px solid var(--border-glass); }
...
.icon-small.filled {
  color: var(--ocean-blue); /* Keep icons neutral unless highlighted */
}
...
.waiting-state {
  ...
  text-shadow: 0 0 5px var(--ocean-blue-dim);
}
...
.pulse-ring {
  ...
  border-radius: 50%;
  border: 1px solid var(--ocean-blue);
  box-shadow: 0 0 10px var(--ocean-blue-dim);
  animation: ripple 2s infinite;
}

@keyframes ripple {
  0% { transform: scale(0.8); opacity: 1; border-color: var(--ocean-blue); }
...
.log-content::-webkit-scrollbar-thumb { background: rgba(0, 229, 255, 0.3); border-radius: 2px; }
