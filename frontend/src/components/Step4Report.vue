            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (Complete，Complete) -->
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    //  - ": X"
    const factMatch = text.match(/:\s*(\d+)/)
    const entityMatch = text.match(/:\s*(\d+)/)
    const relMatch = text.match(/:\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    //  - ，
    const subQSection = text.match(/### \n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // 
    const nodesMatch = text.match(/:\s*(\d+)/)
    const edgesMatch = text.match(/:\s*(\d+)/)
    const activeMatch = text.match(/:\s*(\d+)/)
    ...
    //  - ，
    const activeSection = text.match(/### 【】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // （ "5 / 9 Agent"）
    const countMatch = text.match(/\*\*:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        } else if (currentName && line.trim() && !line.match(/^|^|^/)) {
          // （）
    ...
    // 
    const interviewBlocks = text.split(/####  #\d+:/).slice(1)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // TwitterReddit
        const twitterMatch = answerText.match(/【Twitter】\n?([\s\S]*?)(?=【Reddit】|$)/)
        const redditMatch = answerText.match(/【Reddit】\n?([\s\S]*?)$/)
        ...
        if (!twitterMatch && redditMatch) {
          //  Reddit ，
          if (interview.redditAnswer && interview.redditAnswer !== '（）') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '（）') {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  //  loading Status
...
.main-title {
  ...
  font-weight: 700;
  color: var(--ocean-blue);
  text-shadow: 0 0 10px var(--ocean-blue-dim);
  line-height: 1.2;
  margin: 0 0 16px 0;
  letter-spacing: -0.02em;
}
...
.section-number {
  ...
  font-size: 16px;
  color: var(--text-secondary); /* ，Status */
  font-weight: 500;
}
...
.generated-content :deep(.md-h2) { font-size: 20px; border-bottom: 1px solid var(--border-glass); padding-bottom: 8px; }
...
.right-panel {
  ...
  --wf-border: var(--border-glass);
  --wf-divider: rgba(0, 229, 255, 0.1);

  --wf-active-bg: rgba(0, 229, 255, 0.05);
  --wf-active-border: var(--ocean-blue);
  --wf-active-dot: var(--ocean-blue);
  --wf-active-text: var(--ocean-blue);

  --wf-done-bg: rgba(0, 255, 136, 0.05);
  --wf-done-border: rgba(0, 255, 136, 0.3);
  --wf-done-dot: #00ff88;

  --wf-muted-dot: rgba(255, 255, 255, 0.2);
  --wf-todo-text: var(--text-secondary);
}
