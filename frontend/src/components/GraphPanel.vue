...
const entityTypes = computed(() => {
  ...
  const typeMap = {}
  // Neon telemetry color palette
  const colors = ['#00e5ff', '#ff3366', '#00ff88', '#ff9900', '#bb00ff', '#00bfff', '#ff00aa', '#00ffcc']
...
const closeDetailPanel = () => {
  ...
  expandedSelfLoops.value = new Set() // Status
}
...
const renderGraph = () => {
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      // 
      d3.select(event.target).attr('stroke', 'var(--ocean-blue)').attr('stroke-width', 3)
  ...
  const linkLabelBg = linkGroup.selectAll('rect')
    ...
    .on('click', (event, d) => {
      event.stopPropagation()
      linkGroup.selectAll('path').attr('stroke', 'rgba(0, 229, 255, 0.3)').attr('stroke-width', 1.5)
      linkLabelBg.attr('fill', 'rgba(5, 10, 20, 0.8)')
      linkLabels.attr('fill', 'var(--text-secondary)')
      // 
      link.filter(l => l === d).attr('stroke', 'var(--ocean-blue)').attr('stroke-width', 3)
      d3.select(event.target).attr('fill', 'rgba(0, 229, 255, 0.2)')
  ...
  const linkLabels = linkGroup.selectAll('text')
    ...
    .on('click', (event, d) => {
      ...
      // 
      link.filter(l => l === d).attr('stroke', 'var(--ocean-blue)').attr('stroke-width', 3)
      d3.select(event.target).attr('fill', 'var(--ocean-blue)')
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .on('click', (event, d) => {
      ...
      linkGroup.selectAll('path').attr('stroke', 'rgba(0, 229, 255, 0.3)').attr('stroke-width', 1.5)
      // 
      d3.select(event.target).attr('stroke', 'var(--accent-red)').attr('stroke-width', 4)
      // 
...
.panel-header {
  ...
  align-items: center;
  background: linear-gradient(to bottom, rgba(5, 10, 20, 0.95), rgba(5, 10, 20, 0));
  pointer-events: none;
}
...
@keyframes breathe {
  0%, 100% { opacity: 0.7; transform: scale(1); filter: drop-shadow(0 0 2px rgba(0, 229, 255, 0.3)); }
  50% { opacity: 1; transform: scale(1.15); filter: drop-shadow(0 0 8px rgba(0, 229, 255, 0.6)); }
}
